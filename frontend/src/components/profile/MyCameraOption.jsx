...
function MyCameraOption({ selectedVideo, setSelectedVideo, selectedAudioInput, setSelectedAudioInput, myVideoRef }) {
  // 장치 정보들을 담는 변수
  const [audioInputDevices, setAudioInputDevices] = useState([]);
  const [audioOutputDevices, setAudioOutputDevices] = useState([]);
  const [videoDevices, setVideoDevices] = useState([]);
  // 부모 컴포넌트에 카메라/마이크값만 있으니, 스피커값 변수도 추가  
      ...
      <div className="select" style={{ marginBottom: '10px'}}>
        ...
        <select
          ...
          onChange={(e) => {
            ...
            attachSinkId(myVideoRef.current, e.target.value)
        }}
        >
          <option value="">장치를 선택하세요</option>
